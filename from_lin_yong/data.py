...
class SubDataset(object):
    def __init__(self, x_array, y_array, env_array, transform):
...
class SpuriousValDataset(Dataset):
    ...
    def __getitem__(self, idx):
        x, y, g = self.val_dataset.__getitem__(idx)
        g = g * 0
        return x, y, g

class CifarMnistSpuriousDataset(Dataset):
    def __init__(self, train_num,test_num,cons_ratios, cifar_classes=(1, 9),train_envs_ratio=None, label_noise_ratio=None, augment_data=True, color_spurious=False, transform_data_to_standard=1):
        self.cons_ratios=cons_ratios
        self.train_num = train_num
        self.test_num = test_num
        self.train_envs_ratio=train_envs_ratio
        self.augment_data = augment_data
        self.x_array, self.y_array, self.env_array = \
            get_mnist_cifar_env(
                train_num=train_num,
                test_num=test_num,
...
def get_data_loader_cifarminst(batch_size, train_num, test_num, cons_ratios, train_envs_ratio, label_noise_ratio=None, augment_data=True, cifar_classes=(1, 9), color_spurious=False, transform_data_to_standard=1):
